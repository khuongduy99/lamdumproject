  ...
  if(objects.length>0){
    ...
    jQuery.each( objects, function( key,curObj ) {
      ...
      var pagination = '<div class="hip-pagination" id="hip_page">' +
        '<a href="#prev" class="prev_btn"><i class="fas fa-chevron-left"></i></a>';
      for (var i = 1; i<=page_length; i++)
      {
        if (i===1)
          pagination += '<a class="page-num active" href="#'+i+'">'+i+'</a>';
        else
          pagination += '<a class="page-num" href="#'+i+'">'+i+'</a>';
      }
      pagination += '<a href="#next" class="next_btn"><i class="fas fa-chevron-right"></i></a>\n' +
        '</div>';
